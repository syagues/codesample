(() => {
  ...
  var generateHtmls = (codeSample2) => {
    let html = "";
    for (const sampleKey in codeSample2.samples) {
      const sample = codeSample2.samples[sampleKey];
      html += `<div class="cs-sample" cs-id="${sampleKey}">${generateSampleHtml(sample)}</div>`;
    }
    html = `<pre><code class="cs-wrapper">${html}</code></pre>`;
    return html;
  };
  var generateSampleHtml = (sample) => {
    const rawHtml = import_prismjs.default.highlight(sample.content, import_prismjs.default.languages[sample.language], sample.language);
    return rawHtml;
  };
